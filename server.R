    ...
    #viz1
     ...
     output$viz2_desc <- renderText({
          if (input$select == 1) {
               ...
               "This pie chart shows the distribution of Oscar wins by gender. In the pie plotly above, we decided to represent the proportions of race diversity at the Oscars. Pie charts are a great way to organize and show data by representing the different slices as a percentage of the whole. By using the widget, you can switch between the race-based graph and the gender-based graph. Our pie chart slices are broken up into two recorded genders: Male and Female. Through this, we were able to identify that the majority of the winners/nominees were White. When hovering over the pie section, the exact value can be read."
          }
     })
